import torch
import os
import torch.nn.functional as F
from .base_model import BaseModel
from .networks.classifier import FcClassifier
from .networks.lstm_encoder import LSTMEncoder

class BaselineRawModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument('--max_seq_len', type=int, default=100, help='max sequence length of lstm')
        parser.add_argument('--regress_layers', type=str, default='256,128', help='256,128 for 2 layers with 256, 128 nodes respectively')
        parser.add_argument('--hidden_size', default=256, type=int, help='lstm hidden layer')
        parser.add_argument('--dropout_rate', default=0.5, type=float, help='drop out rate of FC layers')
        parser.add_argument('--target', default='arousal', type=str, help='one of [arousal, valence]')
        return parser

    def __init__(self, opt, logger=None):
        """Initialize the LSTM autoencoder class

        Parameters:
            opt (Option class)-- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        super().__init__(opt, logger)
        self.loss_names = ['MSE']
        self.model_names = ['_seq', '_reg']
        # net seq
        if opt.hidden_size == -1:
            opt.hidden_size = min(opt.input_dim // 2, 512)
        self.net_seq = LSTMEncoder(opt.input_dim, opt.hidden_size)
        # net regression
        layers = list(map(lambda x: int(x), opt.regress_layers.split(',')))
        self.hidden_size = opt.hidden_size
        self.net_reg = FcClassifier(opt.hidden_size, layers, 1, dropout=opt.dropout_rate)
        # settings 
        self.target_name = opt.target
        self.max_seq_len = opt.max_seq_len
        if self.isTrain:
            self.criterion_reg = torch.nn.MSELoss(reduction='mean')
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            paremeters = [{'params': getattr(self, 'net'+net).parameters()} for net in self.model_names]
            self.optimizer = torch.optim.Adam(paremeters, lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizers.append(self.optimizer)
    
    def set_input(self, input):
        """Unpack input data from the dataloader and perform necessary pre-processing steps.

        Parameters:
            input (dict): include the data itself and its metadata information.

        """
        self.feature = input['feature'].to(self.device)
        self.target = input[self.target_name].to(self.device)
        self.mask = input['mask'].to(self.device)
        self.length = input['length']

    def forward(self):
        batch_size = self.target.size(0)
        previous_h = torch.zeros(1, batch_size, self.hidden_size).float().to(self.device) 
        previous_c = torch.zeros(1, batch_size, self.hidden_size).float().to(self.device) 
        hidden, _ = self.net_seq(self.feature, (previous_h, previous_c))
        self.pred, _ = self.net_reg(hidden)
        self.pred = self.pred.squeeze()
        self.output = self.pred
    
    def backward(self):
        batch_size = self.target.size(0)
        pred = self.pred * self.mask
        target = self.target * self.mask
        self.loss_MSE = self.criterion_reg(pred, target) 
        self.loss_MSE.backward()    
        for model in self.model_names:
            torch.nn.utils.clip_grad_norm_(getattr(self, 'net'+model).parameters(), 5)

    def run(self):
        """After feed a batch of samples, Run the model."""
        self.forward()
        if self.isTrain:
            self.optimizer.zero_grad()
            self.backward()
            self.optimizer.step()

...
class BaseModel(ABC):
    ...
    To create a subclass, you need to implement the following five functions:
        ...
        -- <run>:                           produce intermediate results, if self.isTrain, then the network will be trained.
    ...
    def __init__(self, opt, logger=None):
        ...
        self.logger = logger    # used for print log message in model
    ...
    @abstractmethod
    def run(self):
        """After feed a batch of samples, Run the model and get results."""
        pass
    ...
    def test(self):
        """run function used in test time.

        This function wraps <run> function in no_grad() so we don't save intermediate steps for backprop
        It also calls <compute_visuals> to produce additional visualization results
        """
        cur_isTrain = self.isTrain
        self.isTrain = False
        with torch.no_grad():
            self.run()
            self.compute_visuals()
        self.isTrain = cur_isTrain
    ...
    def load_networks_folder(self, folder_path):

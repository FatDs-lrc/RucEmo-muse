...
class SeqTextCNN(nn.Module):
    def __init__(self, input_dim, emb_size=128, in_channels=1, out_channels=128, kernel_heights=[3,4,5], dropout=0.5):
        super().__init__()
        '''
        cat((conv1-relu+conv2-relu+conv3-relu)+maxpool) + dropout, and to trans
        '''
        self.conv1 = nn.Conv2d(in_channels, out_channels, (kernel_heights[0], input_dim), stride=1, padding=self.get_padding_size(kernel_heights[0]))
        self.conv2 = nn.Conv2d(in_channels, out_channels, (kernel_heights[1], input_dim), stride=1, padding=self.get_padding_size(kernel_heights[1]))
        self.conv3 = nn.Conv2d(in_channels, out_channels, (kernel_heights[2], input_dim), stride=1, padding=self.get_padding_size(kernel_heights[2]))
        self.dropout = nn.Dropout(dropout)
        self.embd = nn.Sequential(
            nn.Linear(len(kernel_heights)*out_channels, emb_size),
            nn.ReLU(inplace=True),
    ...
    def get_padding_size(self, kernel_height):
        return (kernel_height // 2, 0)
    ...
    def forward(self, frame_x):
        batch_size, seq_len, feat_dim = frame_x.size()
        frame_x = frame_x.view(batch_size, 1, seq_len, feat_dim)
        out1 = self.conv_block(frame_x, self.conv1)
        out2 = self.conv_block(frame_x, self.conv2)
        out3 = self.conv_block(frame_x, self.conv3)
        all_out = torch.cat((out1, out2, out3), 1).transpose(-1, -2)
        fc_in = self.dropout(all_out)
        embd = self.embd(fc_in)
        return embd

if __name__ == '__main__':
    a = SeqTextCNN(300, 128, kernel_heights=[3,5,7])
    x = torch.rand(32, 100, 300)
    out = a(x)
    print(out.shape)
